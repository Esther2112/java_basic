...
public class MemberView {
    ...
    void removeMemberViewProcess(){
        //삭제대상 이메일 입력받기
        String email = input("# 삭제할 대상의 이메일: ");
        //존재하는지 확인 후 삭제 처리 위임
        Member foundMember = mr.findByEmail(email);
        if(foundMember != null){
            char answer = input(String.format("# %s님의 정보를 정말 삭제하시겠습니까? [y/n] : ", foundMember.memberName)).toLowerCase().charAt(0);
            if(answer == 'y') {
    ...
    void signUp() {
        ...
        Member newMember = new Member(
                email, password,
                name, mr.getLastMemberId() + 1, gender, age
        );
